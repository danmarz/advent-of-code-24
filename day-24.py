...
for input_line in inputs.split("\n"):
    input_name, input_value = input_line.split(": ")
    gates[input_name] = int(input_value)

# Parse outputs
instructions = {}
for output_line in outputs.split("\n"):
    expression, result = output_line.split(" -> ")
    instructions[result] = expression.split(" ")


# Recursive evaluation function
def evaluate(gate):
    if gate in gates:  # Already evaluated
        return gates[gate]
    expression = instructions[gate]
    if "AND" in expression:
        gates[gate] = and_gate(evaluate(expression[0]), evaluate(expression[2]))
    elif "OR" in expression:
        gates[gate] = or_gate(evaluate(expression[0]), evaluate(expression[2]))
    elif "XOR" in expression:
        gates[gate] = xor_gate(evaluate(expression[0]), evaluate(expression[2]))
    else:  # Direct assignment
        gates[gate] = evaluate(expression[0])
    return gates[gate]
...
gates = OrderedDict(sorted(gates.items()))
result = [gates[gate] for gate in gates if gate.startswith("z")]

print(result[::-1])  # Reverse for correct order
print(int("".join(map(str, result[::-1])), 2))
...
# Credits for this solution go to https://www.reddit.com/r/adventofcode/comments/1hl698z/comment/m3kt1je/ :)
