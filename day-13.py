...
for pos, section in enumerate(sections):
    # print(pos, section.split("\n"))
    lines = section.split("\n")
    # machines[pos] = {
    #     "buttonAx": int(lines[0].split(",")[0].split("+")[1]),
    #     "buttonAy": int(lines[0].split(",")[1].split("+")[1]),
    #     "buttonBx": int(lines[1].split(",")[0].split("+")[1]),
    #     "buttonBy": int(lines[1].split(",")[1].split("+")[1]),
    #     "prizeX": int(lines[2].split(",")[0].split("=")[1]),
    #     "prizeY": int(lines[2].split(",")[1].split("=")[1]),
    # }
    buttonAx = int(lines[0].split(",")[0].split("+")[1])
    buttonAy = int(lines[0].split(",")[1].split("+")[1])
    buttonBx = int(lines[1].split(",")[0].split("+")[1])
...
#     # Split by double newlines to separate machines
#     machines_raw = [m.strip() for m in input_str.strip().split("\n\n")]
#     machines = []
...
#         a_line = lines[0]
#         # Extract integers
#         # We'll split by spaces and then parse the parts after 'X+' and 'Y+'
#         # Format: Button A: X+94, Y+34
#         a_parts = a_line.replace("Button A: ", "").split(",")
#         aX_str = a_parts[0].strip().replace("X+", "").replace("X-", "-")
#         aY_str = a_parts[1].strip().replace("Y+", "").replace("Y-", "-")
#         aX = int(aX_str)
#         aY = int(aY_str)

#         # Parse Button B
#         b_line = lines[1]
#         b_parts = b_line.replace("Button B: ", "").split(",")
#         bX_str = b_parts[0].strip().replace("X+", "").replace("X-", "-")
#         bY_str = b_parts[1].strip().replace("Y+", "").replace("Y-", "-")
#         bX = int(bX_str)
#         bY = int(bY_str)
...
#         p_parts = p_line.replace("Prize: ", "").split(",")
#         pX_str = p_parts[0].strip().replace("X=", "")
#         pY_str = p_parts[1].strip().replace("Y=", "")
#         pX = int(pX_str)
#         pY = int(pY_str)
...
def find_minimum_cost_for_machine(aX, aY, bX, bY, pX, pY, max_presses=100):
    # We need to solve:
    # aX * a + bX * b = pX
    # aY * a + bY * b = pY
    #
    # for non-negative integers a, b where 0 <= a,b <= max_presses.

    min_cost = None
    best_solution = None

    # Cost of pressing A is 3 tokens, B is 1 token
    costA = 3
    costB = 1
...
for aX, aY, bX, bY, pX, pY in machines:
    cost, solution = find_minimum_cost_for_machine(aX, aY, bX, bY, pX, pY)
    if cost is not None:
        prizes_won += 1
        total_cost += cost

print(f"Part 1 prizes won: {prizes_won}")
print(f"Part 1 minimum total cost: {total_cost}")
...
for pos, section in enumerate(sections):
    ...
    buttonBx = int(lines[1].split(",")[0].split("+")[1])
    buttonBy = int(lines[1].split(",")[1].split("+")[1])
    prizeX = int(lines[2].split(",")[0].split("=")[1]) + 10000000000000
    prizeY = int(lines[2].split(",")[1].split("=")[1]) + 10000000000000
    machines.append((buttonAx, buttonAy, buttonBx, buttonBy, prizeX, prizeY))
...
def solve_machine(aX, aY, bX, bY, pX, pY):
    ...
    det = aX * bY - aY * bX
    if det == 0:
        # No unique solution or infinite solutions without a unique integral solution
        return None
    ...
    # We must have nonnegative integers
    if a < 0 or b < 0:
        return None
...
print(f"Part 2 prizes won: {prizes_won}")
print(f"Part 2 minimum total cost: {total_cost}")
