...
def getpos(data, c):
    return next((i, j) for i in range(h) for j in range(w) if data[i][j] == c)
...
def part1(pd):
    return sum(
        (px + 2 * dix, py + 2 * diy) in pd
        and pd[(px, py)] + 102 <= pd[(px + 2 * dix, py + 2 * diy)]
        for px, py in pd
        for dix, diy in dirs
    )
...
def getshortcuts(pl, i, di=20, sh=100):
    ...
    while j < len(pl):
        ...
        if (dist <= di) and (gain >= sh):
            steps = di - dist + 1
            for k in range(min(len(pl) - j, steps)):
                ret.add(pl[j + k])
            j += steps
        elif dist <= di:
            j += (sh + 1 - gain) // 2
...
def solve(pl, pd, di=20, sh=100):
    ...
    for i in range(1, len(pl) - sh - 2):
        ...
        for j in range(-di, di + 1):
            ...
            if (nex, ney) in pd:
                k = pd[(nex, ney)]
                if k - i - mand(*pl[i], nex, ney) >= sh:
                    s.add((nex, ney))
        for j in range(i + sh + 1, min(len(pl), i + sh + di)):
            if j - i - mand(*pl[i], *pl[j]) < sh:
                s.discard(pl[j])
        ret += len(s)
    return ret
